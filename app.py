...
PLOT_TRANSLATIONS = {
    ...
    "تقاضای برنامه‌ریزی‌شده (Z)": "Planned Demand (Z)",
    ...
    "تقاضای مبنا": "Baseline Demand",
    "تقاضای جاری (Z)": "Current Demand (Z)",
    "IS مبنا": "Baseline IS",
    ...
    "تقاضای کل پول Mᵈ/P = L(r, Y₁)": "Total Money Demand Mᵈ/P = L(r, Y₁)",
    "تقاضای پول مبنا (L₀)": "Baseline Money Demand (L₀)",
    "تقاضای مبنا L₀(r, Y₀)": "Baseline Money Demand L₀(r, Y₀)",
    "تقاضای کلاسیک عمودی (Mᵈ/P = kY)": "Classical Money Demand (Mᵈ/P = kY)",
    "تقاضا در دام نقدینگی (کشش بی‌نهایت در کف)": "Liquidity Trap Demand (Horizontal)",
    "مسیر پله‌ای تعدیل موجودی انبار": "Inventory Adjustment Path",
    "تعادل اولیه E₀": "Initial Equilibrium E₀",
    "تعادل اولیه": "Initial Equilibrium",
    "تعادل جدید E₁": "New Equilibrium E₁",
    "تعادل نهایی E₁": "Final Equilibrium E₁",
...
def translate_figure_to_en(fig):
    """ترجمه زنده المان‌های گرافیکی Plotly به انگلیسی"""
    ...
    if hasattr(fig.layout, "annotations"):
        for ann in fig.layout.annotations:
            ...
            if "تقاطع کینزی" in txt:
                ann.text = "A) Keynesian Cross & Adjustment"
...
# اعمال استایل راست‌به‌چپ یا چپ‌به‌راست و رفع باگ بسته شدن سایدبار
st.markdown(f"""
<style>
...
if "IS" in topic or "کالا" in topic:
    ...
    st.sidebar.subheader(T["dynamics"]["settings_title"])
    show_cobweb = st.sidebar.checkbox(
        T["dynamics"]["cobweb_checkbox"],
        value=True
    )
...
# ==========================================
# فصل ۲: بازار پول (منطبق بر کتاب مدرسان شریف)
# ==========================================
    ...
    with tab4:
        if m_regime == "liquidity_trap":
            st.info("💡 **دام نقدینگی کینزی:** نرخ بهره به کف حداقلی رسیده و ترجیح نقدینگی بی‌نهایت است. تزریق پول توسط بانک مرکزی جذب مانده‌های راکد شده و نرخ بهره را پایین‌تر نمی‌برد.")
        elif m_regime == "classical":
            st.warning("💡 **حالت کلاسیک:** پول صرفاً نقش واسطه مبادله دارد و تقاضای سفته‌بازی صفر است ($h=0$). نرخ بهره در بازار سرمایه حقیقی تعیین می‌شود.")
        else:
